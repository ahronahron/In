...
<head>
    ...
    <title>Reports & Analytics - PHARMACY Management System</title>
...
<body class="min-h-screen bg-background transition-colors duration-300">
    ...
    <!-- Main Content - Include the rest from reports_analytics.html -->
    <?php
    // Read the HTML file and extract main content (after </head>)
    $html_file = __DIR__ . '/reports_analytics.html';
    if (file_exists($html_file)) {
        ...
        if (preg_match('/<\/head>(.*?)<\/body>/s', $content, $matches)) {
            ...
            $main_content = preg_replace('/<!-- Sidebar.*?<\/aside>/s', '', $main_content);
            // Update links
